...
pheatmap(log10(countdata[select,]+1), 
         show_rownames=TRUE, 
         cluster_cols=TRUE, 
         annotation_col=sampleinfo[ , c("condition.name","sexe")] ,
         scale = "row",
...
Now we have access to two more variables, `sexe` and `condition.name`, that we could use to form groups.

```{r}
ggplot(df_melt, aes(x = sexe, y = value_norm_log, fill =sexe)) +
  geom_bar(alpha = 0.5, stat = "summary", fun.y = "median", position = "dodge") + 
  facet_wrap(~condition.name)
```
...
ggplot(df_melt[idx_select, ], aes(x = sexe, y = value_norm_log, fill = sexe)) +
  ...
  geom_signif(comparisons = list(1:2), 
              ...
              position = "identity") +
  facet_grid(GeneID~condition.name)
```
...
pheatmap(data.norm.log[idx_select, ],
         show_rownames=TRUE,
         annotation_col=sampleinfo[ , c("sexe","condition.name")],
...
Color points according to the sample `condition.name` or `sexe`.
...
dds <- DESeqDataSetFromMatrix(countData = countdata,
                              colData = sampleinfo,
                              design = ~ sexe + condition.name)
```

The design indicates how to model the samples, i.e how the counts for each gene depend on the variables in `colData = sampleinfo`. Here we want to measure the effect of the cell type and of the pregnancy condition.name of the mice. Note that the two factor variables `sexe` and `condition.name` should be columns of `sampleinfo`.
...
```{r}
res <- results(dds, alpha = 0.05, pAdjustMethod="BH", name="condition.name_quiescent_vs_active")
head(res)
```
...
```{r}
#get normalized data
d <- plotCounts(dds, gene=as.character(enrich$ID[1]), intgroup=c("sexe","condition.name"), 
                returnData=TRUE)

library("ggplot2")
p <- ggplot(d, aes(x=condition.name, y=log10(count+1))) + 
  geom_point(position=position_jitter(w=0.1,h=0))  +
  facet_wrap(~sexe)
p
```

We can also facet the plot according to `condition.name`. It could be interesting to see how the `sexe` effect depends on `condition.name`. See section [Interactions] to address such questions.

```{r}
ggplot(d, aes(x=sexe, y=log10(count+1))) + 
  geom_point(position=position_jitter(w=0.1,h=0))  +
  facet_wrap(~condition.name)
```
...
### Interactions
Interaction terms allow to test, for example, if the log2 fold change attributable to a given condition is different based on another factor, for example if the `sexe` effect differs across `condition.name`.
...
```{r}
dds.int <- dds
dds.int$group <- factor(paste(dds.int$condition.name, dds.int$sexe, sep="."))
design(dds.int) <- ~ group
dds.int <- DESeq(dds.int)
resultsNames(dds.int)
```

Now we can compare active and quiescent patients that received the TNF sexe.
```{r}
